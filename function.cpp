...
int func::func_zoom_trans(QImage *img,double xx,double yy,QString color){
    int x,y;
    x=(int)xx;
    y=(int)yy;
    int a,b,c,d;
    ...
    default:{
        QMessageBox::information(NULL,MainWindow::tr("Fail"),MainWindow::tr("the type is error"));
        return -1;
    }
    }
    int A,B,C,D;
...
QImage * func::func_zoom_in_out(QImage *img ,double coef){
       int w,h;
...
//       qDebug()<<"wi"<<img->width()<<"\n"<<img->height();
       ...
       for(int y=0;y<h;y++){
           for(int x=0;x<w;x++){
               ...
               xx=(double)x/coef;
               yy=(double)y/coef;
               int r=func::func_zoom_trans(img,xx,yy,"red");
               int g=func::func_zoom_trans(img,xx,yy,"green");
               int b=func::func_zoom_trans(img,xx,yy,"blue");
